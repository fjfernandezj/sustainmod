# Mapping regiones, comunas, provincias
...
####--Region del Bio bio antes de creación region del ñuble en 2017

# r_biobio <- c("c_arauco", "c_canete", "c_contulmo", "c_curanilahue", "c_lebu", "c_los_alamos","c_tirua", "c_alto_biobio",
#              "c_antuco", "c_cabrero", "c_laja", "c_los_angeles", "c_mulchen", "c_nacimiento", "c_negrete", "c_quilaco",
#              "c_quilleco", "c_san_rosendo", "c_santa_barbara", "c_tucapel", "c_yumbel", "c_chiguayante", "c_concepcion",
#              "c_coronel", "c_florida", "c_hualpen", "c_hualqui", "c_lota", "c_penco", "c_san_pedro_de_la_paz", "c_santa_juana",
#              "c_talcahuano", "c_tome", "c_cobquecura", "c_coelemu", "c_ninhue", "c_portezuelo", "c_quirihue", "c_ranquil",
#              "c_treguaco", "c_bulnes", "c_chillan", "c_chillan_viejo", "c_el_carmen", "c_pemuco", "c_pinto", "c_quillon",
#              "c_san_ignacio", "c_yungay", "c_coihueco", "c_niquen", "c_san_carlos", "c_san_fabian", "c_san_nicolas")
...
r_araucania <- c("c_angol", "c_collipulli", "c_curacautin", "c_ercilla", "c_lonquimay", "c_los_sauces", "c_lumaco",
                 "c_puren", "c_renaico", "c_traiguen", "c_victoria", "c_gorbea", "c_lautaro", "c_loncoche", "c_melipeuco", "c_nueva_imperial", "c_padre_las_casas", "c_perquenco",
...
# p_nuble <- c("c_cobquecura", "c_coelemu", "c_ninhue", "c_portezuelo", "c_quirihue", "c_ranquil",
#             "c_treguaco", "c_bulnes", "c_chillan", "c_chillan_viejo", "c_el_carmen", "c_pemuco", "c_pinto", "c_quillon",
#             "c_san_ignacio", "c_yungay", "c_coihueco", "c_niquen", "c_san_carlos", "c_san_fabian", "c_san_nicolas")
...
### Región de aysen
p_coyhaique <- c("c_coyhaique", "c_lago_verde")
...
provincias_df <- tibble(
  comuna = c(p_arica, p_parinacota, p_iquique, p_el_tamarugal, p_antofagasta, p_el_loa, p_tocopilla, p_chanaral, p_copiapo, p_huasco, p_choapa, p_elqui, p_limari, p_isla_de_pascua, p_los_andes, p_petorca,
             p_quillota, p_san_antonio, p_san_felipe, p_valparaiso, p_marga_marga, p_chacabuco,
  ...
  provincia = rep(c("p_arica", "p_parinacota", "p_iquique", "p_el_tamarugal", "p_antofagasta",
                    ...
                    "p_petorca", "p_quillota", "p_san_antonio", "p_san_felipe", "p_valparaiso",
                    "p_marga_marga", "p_chacabuco", "p_cordillera", "p_maipo", "p_melipilla",
                    "p_santiago", "p_talagante", "p_cachapoal", "p_cardenal_caro", "p_colchagua",
                  ...
                  times = c(length(p_arica), length(p_parinacota), length(p_iquique), 
                            ...
                            length(p_huasco), length(p_choapa), length(p_elqui),length(p_limari),
                            length(p_isla_de_pascua), length(p_los_andes), length(p_petorca), 
                            length(p_quillota), length(p_san_antonio), length(p_san_felipe),
                            length(p_valparaiso), length(p_marga_marga), length(p_chacabuco),
                            length(p_cordillera), length(p_maipo),length(p_melipilla),
                            length(p_santiago), length(p_talagante), length(p_cachapoal),
                            length(p_cardenal_caro), length(p_colchagua), length(p_cauquenes),
                            length(p_curico), length(p_linares), length(p_talca), length(p_arauco),
                            length(p_biobio), length(p_concepcion),length(p_diguillin),
